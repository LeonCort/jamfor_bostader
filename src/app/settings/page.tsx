...
export default function SettingsPage() {
  ...
  useEffect(() => {
    if (places && places.length > 0) {
      setPlaceRows(places.map((p) => ({ id: p.id, label: p.label ?? "", address: p.address ?? "", icon: p.icon, arriveBy: p.arriveBy, leaveAt: p.leaveAt })));
    } else {
  ...
  return (
    <div className="mx-auto max-w-screen-2xl px-4 sm:px-6 py-6">
      ...
      <div className="grid gap-6 sm:grid-cols-[220px_1fr] items-start">
        {/* Left sidebar */}
        <aside className="rounded-2xl border border-border/60 bg-card/80">
          <nav aria-label="Inställningar" role="tablist" aria-orientation="vertical" className="p-2 sm:p-3 flex sm:flex-col gap-1">
            <button
              type="button"
              role="tab"
              aria-selected={active === 'current'}
              className={`w-full text-left px-3 py-2 rounded-md text-sm transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-ring ${active === 'current' ? 'bg-muted text-foreground' : 'hover:bg-muted/60 text-muted-foreground'}`}
            ...
            <button
              ...
              className={`w-full text-left px-3 py-2 rounded-md text-sm transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-ring ${active === 'places' ? 'bg-muted text-foreground' : 'hover:bg-muted/60 text-muted-foreground'}`}
        ...
        {/* Right content */}
        <div role="tabpanel" aria-labelledby={active === 'current' ? 'current' : 'places'} className="space-y-6">
          {active === 'current' ? (
            ...
            <section className="rounded-2xl border border-border/60 bg-card/80 p-4 sm:p-6">
              ...
              <div className="mt-4 space-y-3">
                {placeRows.map((row, idx) => (
                  <div key={row.id ?? idx} className="grid grid-cols-1 sm:grid-cols-[1fr_1fr_1fr_1fr_auto] gap-2 items-end border border-border/60 rounded-md p-3">
                    <div className="space-y-1">
                      <Label>Etikett</Label>
                      <Input
                        value={row.label}
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        type="time"
                        value={row.arriveBy ?? ''}
                        onChange={(e) => setPlaceRows((rs) => rs.map((r, i) => i === idx ? { ...r, arriveBy: e.target.value } : r))}
                      />
                    </div>
                    <div className="space-y-1">
                      ...
                      <Input
                        type="time"
                        value={row.leaveAt ?? ''}
                        onChange={(e) => setPlaceRows((rs) => rs.map((r, i) => i === idx ? { ...r, leaveAt: e.target.value } : r))}
                      />
                    </div>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      const cleaned = placeRows
                        .map((p) => ({ id: p.id, label: p.label?.trim() || undefined, address: p.address?.trim() || undefined, icon: p.icon?.trim() || undefined, arriveBy: p.arriveBy?.trim() || undefined, leaveAt: p.leaveAt?.trim() || undefined }))
